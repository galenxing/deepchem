...
from deepchem.models.tensorgraph.layers import Input, Constant, GraphConv, Add, SluiceLoss, BatchNorm, GraphPool, Dense, GraphGather, BetaShare, LayerSplitter, SoftMax, Multiply, SoftMaxCrossEntropy, Concat, WeightedError, Label, Weights, Feature, AlphaShare
...
def hard_param_mt(batch_size, tasks):
  ...
  for task in tasks:
    classification = Dense(out_channels=20, activation_fn=None, in_layers=[d3a])
    label = Label(shape=(None, 20))
    softmax = SoftMax(in_layers=[classification])
    model.add_output(softmax)
