...
def three_layer_dense(batch_size, tasks):
  model = TensorGraph(
      model_dir='graphconv/three_layer_dense' + current_milli_time(),
...
def hard_param_mt(batch_size, tasks):
  model = TensorGraph(
      model_dir='graphconv/hard_param_mt',
      batch_size=batch_size,
      use_queue=False,
      tensorboard=True)
  sluice_cost = []
...
def two_layer_sluice(batch_size, tasks, minimizer):
  model = TensorGraph(
      model_dir='graphconv/two_layer_sluice/' + str(minimizer) +
  ...
  d2a.set_summary(name='d2a', summary_op='histogram')
  ...
  for task in tasks:
    if count < len(tasks) / 2:
      ...
      label = Label(shape=(None, 20), name='label1')
    else:
      classification = Dense(
