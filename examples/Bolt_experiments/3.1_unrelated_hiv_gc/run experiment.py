...
for count, train in enumerate(train_datasets):
  for weight in weights:
    ...
    train_scores = model.evaluate(
        train,metrics= [metric], transformers=[], per_task_metrics=True)
    valid_scores = model.evaluate(
        valid_dataset,metrics= [metric], transformers=[], per_task_metrics=True)
    ...
    record_info(
        file_name='3.1_unrelated_hiv_gc_1.csv',
        train=train_scores,
        valid=valid_scores,
        weight=weight,
        epochs=epoch,
