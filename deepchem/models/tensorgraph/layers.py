    ...
    def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
        ...
        if set_tensors:
            ...
            self.rnn_zero_states.append(
                np.zeros(zero_state.get_shape(), np.float32))
        return out_tensor
...
class Input(Layer):
    ...
    def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
        ...
        if len(in_layers) > 0:
            ...
            self.out_tensor = tf.placeholder_with_default(
...
class SoftMaxCrossEntropy(Layer):
    ...
    def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
        ...
        tf.summary.histogram("SoftMaxCrossEntropy", out_tensor)
        if set_tensors:
            self.out_tensor = out_tensor
        return out_tensor
...
class GraphGather(Layer):
    ...
    def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
        ...
        assert (self.batch_size > 1,
                "graph_gather requires batches larger than 1")
...
class WeightedError(Layer):
    ...
    def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
        ...
        tf.summary.scalar("WeightedError", out_tensor)
        if set_tensors:
            self.out_tensor = out_tensor
        return out_tensor
...
class NeighborList(Layer):
    ...
    def compute_nbr_list(self, coords):
        ...
        padded_nbrs = [tf.concat([unique_nbrs, padding], 0)
                       for unique_nbrs in nbrs]
    ...
    def get_cells(self):
        ...
        mesh_args = [tf.range(start, stop, nbr_cutoff)
        ...
        for param in self.radial_params:
            ...
            if not self.atom_types:
                ...
                for j in range(len(self.atom_types)):
                    ...
                    sym.append(tf.reduce_sum(
                        tf.where(cond, rsf, rsf_zeros), 2))
        ...
        if boxsize is not None:
            for atom, atom_tensor in enumerate(atom_tensors):
                ...
                for nbr, nbr_tensor in enumerate(nbrs_tensors):
                    ...
                    _D = tf.subtract(
                        _D, boxsize * tf.round(tf.div(_D, boxsize)))
                    D.append(_D)
        else:
            for atom, atom_tensor in enumerate(atom_tensors):
                nbrs = self.gather_neighbors(X, nbr_tensors[atom], B, N, M, d)
